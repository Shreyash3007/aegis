...
export async function runInterview(doc, yes) {
    const base = {
    ...
    for (const q of QUESTIONS) {
        ...
        switch (q.key) {
            case 'platform':
                ...
                break;
            case 'project_type':
                base.project_type = a === 'brownfield' ? 'brownfield' : 'greenfield';
                break;
            case 'human_lane_cap':
                base.human_lane_cap = Math.max(1, parseInt(a, 10) || 2);
                break;
            case 'autonomy':
                base.autonomy = (['assisted', 'semi', 'full'].includes(a) ? a : 'semi');
                break;
            case 'model_strong':
                base.model_tiers.strong = a;
                break;
            case 'ship_profile':
