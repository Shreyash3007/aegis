...
import { configP, stateP, transP, freshState, appStatePath } from '../lib/state.js';
...
export async function init(args) {
    ...
    if (pi !== -1) {
        const v = args[pi + 1];
        if (!v)
            die(2, '--profile requires a value; usage: aegis init [--profile minimal|standard|full]');
    ...
    if (fs.existsSync(configP)) {
        try {
            preserved = JSON.parse(fs.readFileSync(configP, 'utf8'));
        }
        catch {
            preserved = {};
        }
