...
export function ast(args) {
    ...
    if (sub === 'build') {
        ...
        if (!fs.existsSync(graphP()))
            die(2, 'no stored graph - run aegis ast build first');
        const old = readJ(graphP());
