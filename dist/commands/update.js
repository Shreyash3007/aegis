...
export function latestTag(lsRemote) {
    ...
    if (!tags.length)
        return null;
    const key = (t) => t.replace(/^v/, '').split('.').map((n) => parseInt(n, 10));
...
export function update(args) {
    ...
    if (!repoUrl)
        die(2, 'package.json has no repository.url - cannot self-update');
    ...
    if (!latest)
        die(2, `no semver tags found on ${https}`);
    const latestV = latest.replace(/^v/, '');
    const key = (v) => v.split('.').map((n) => parseInt(n, 10));
    ...
    if (!behind) {
        ok(`aegis ${current} is already at or ahead of the latest tag (${latestV})`);
        return;
    }
