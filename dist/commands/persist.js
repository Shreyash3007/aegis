...
export function checkpoint(args) {
    ...
    if (!quiet)
        ok(`checkpoint ${cp.id} written (git ${gitSha.slice(0, 8)}, ${Object.keys(cp.hashes).length} files hashed)`);
}
export function resume() {
    const dir = cpDir();
    const cps = fs.existsSync(dir) ? fs.readdirSync(dir).sort() : [];
    if (!cps.length)
        die(2, 'no checkpoints - restart from 00a (aegis init)');
    const cp = readJ(path.join(dir, cps[cps.length - 1]));
    const now = brainHashes();
    const drift = [];
    for (const [f, h] of Object.entries(cp.hashes)) {
        if (!now[f])
            drift.push(`DELETED: ${f}`);
        else if (now[f] !== h)
            drift.push(`MODIFIED: ${f}`);
    }
    if (drift.length)
