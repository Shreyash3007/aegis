...
let KNOWN = ['help'];
export function setKnownCommands(cmds) { KNOWN = [...cmds, 'help']; }
...
function docType(file) {
    if (path.basename(file) === 'skill.md')
        return 'master';
    if (file.includes('base-roles'))
        return 'template';
    if (file.includes('specializations'))
        return 'spec';
    if (file.includes('interview-protocol'))
        return 'protocol';
    return 'subskill';
}
...
async function judgeFile(file, key, url, model) {
    try {
        ...
        if (typeof text !== 'string')
            return { status: 'error', reason: 'empty judge response' };
...
export async function evalCmd(args) {
    ...
    if (args[0] === '--all') {
        ...
        walk(skillsDir);
        if (!files.length)
            die(2, 'no skills found - run aegis init first');
    }
    else if (args[0] && fs.existsSync(args[0])) {
        files = [args[0]];
    }
    else
        die(2, 'usage: aegis eval <file.md|--all>');
