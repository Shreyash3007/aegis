...
const SETTABLE: Record<string, (v: string) => unknown> = {
  platform: (v) => v,
  mode: (v) => (v === 'manual' ? 'manual' : 'runtime'),
  autonomy: (v) => { if (!['assisted','semi','full'].includes(v)) die(4, 'autonomy: assisted|semi|full'); return v; },
  environment_level: (v) => { if (!['L0','L1','L2'].includes(v)) die(4, 'level: L0|L1|L2'); return v; },
