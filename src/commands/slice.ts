...
export function slice(args: string[]): void {
  const [op, name] = args;
  const s = loadState() as any;
  s.slices = s.slices ?? {};

  if (op === 'create') {
    if (!name) die(4, 'usage: aegis slice create <name>');
    ...
    const entries = Object.entries(s.slices) as [string, any][];
    if (!entries.length) { console.log('no slices registered'); return; }
  ...
  } else die(4, 'usage: aegis slice <create|list|remove> <name>');
}
