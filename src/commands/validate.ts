...
const suites: Record<string, () => Result | Promise<Result>> = {
  contracts: () => {
    ...
    const STACK_EXT: Record<string, RegExp> = {
    ...
    const walk = (d: string): void => {
      for (const f of fs.readdirSync(d)) {
        ...
        if (fs.statSync(p).isDirectory()) walk(p);
          ...
          if (/\.tsx?$/.test(f)) tsFiles.push(p);
          else if (stack !== 'typescript' && STACK_EXT[stack]?.test(f)) stackFiles.push(p);
        }
      }
    };
    if (fs.statSync(cpAbs).isDirectory()) walk(cpAbs);
    else {
      if (/\.tsx?$/.test(cpAbs)) tsFiles.push(cpAbs);
      else if (stack !== 'typescript' && STACK_EXT[stack]?.test(cpAbs)) stackFiles.push(cpAbs);
    }
