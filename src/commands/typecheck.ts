...
const SRC_EXT: Record<string, RegExp> = {
...
function hasSource(root: string, ext: RegExp): boolean {
  const stack = [root];
  while (stack.length) {
    ...
    for (const f of entries) {
      ...
      if (st.isDirectory()) { if (!SKIP_DIRS.has(f)) stack.push(p); }
      else if (ext.test(f)) return true;
    }
  }
  return false;
}

export function typecheck(_args: string[]): void {
