...
import { AEGIS_DIR, REPO, die, git, gitTry, ok, readJ, writeJ } from '../lib/util.js';
...
function writeHandoff(): void {
  ...
  if (fs.existsSync(cpDir)) {
    ...
    if (cps.length) {
      try {
        const cp = readJ<{ id: string; gitSha: string }>(path.join(cpDir, cps[cps.length - 1]));
        cpId = cp.id;
  ...
  if (apps.length) {
    ...
    for (const a of apps) {
      ...
      try {
        ...
        sk = as.current_skill;
        if (as.fix?.active) fix = ` | fix: "${as.fix.active.desc}"`;
...
export function status(args: string[] = []): void {
  ...
  if (apps.length && !args.includes('--app')) {
    ...
    if (args.includes('--markdown')) writeHandoff();
    return;
  }
