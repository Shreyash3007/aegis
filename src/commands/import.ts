...
export function importCmd(args: string[]): void {
  if (args[0] !== 'check') die(2, 'usage: aegis import check');
  let missing = 0;
  ...
  if (missing) die(4, `import check: ${missing}/${REQUIRED.length} brain docs missing/weak/uncited - finish 00d reverse-discovery`);
