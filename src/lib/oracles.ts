...
export type Stack = 'typescript' | 'rust' | 'go' | 'python' | 'unknown';
...
export function detectStack(cwd: string = REPO): Stack {
  const markers: Array<[string, Stack]> = [
  ...
  for (const [file, stack] of markers) {
    if (fs.existsSync(path.join(cwd, file))) return stack;
  }
  return 'unknown';
}
...
export function typecheckCommand(cwd: string = REPO): string | null {
  switch (detectStack(cwd)) {
    case 'typescript': {
      const localBin = path.join(cwd, 'node_modules', '.bin', 'tsc');
      if (fs.existsSync(localBin)) return `"${localBin}" --noEmit`;
